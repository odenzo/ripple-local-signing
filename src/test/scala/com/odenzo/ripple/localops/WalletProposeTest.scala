...
/** This tests functionality for wallet generation, WalletPropose.
  ...
  */
class WalletProposeTest extends OTestSpec with RippleFormatConverters {
  ...
  test("Regen") {
    val rfc  = "ANTE TUFT MEG CHEN CRAB DUMB COW OWNS ROOF FRED EDDY FORD"
    val seed = getOrLog(WalletGenerator.generateSeedFromSecretKey(rfc))
    val res  = getOrLog(WalletGenerator.generateEdKeys(seed))
  ...
  def doWalletFixture(resource: String): Unit = {
    val loaded      = getOrLog(loadRqRsResource(resource))
    val loadResults = loaded.map(_.rs).traverse(rs => findObjectField("result", rs))

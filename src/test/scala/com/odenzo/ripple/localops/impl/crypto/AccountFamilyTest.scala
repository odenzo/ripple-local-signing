...
class AccountFamilyTest extends FunSuite with OTestSpec with ByteUtils with RippleFormatConverters {
  ...
  test("Wallet Shuffle") {
    ...
    getOrLog(convertBase58Check2hex(master_seed)) shouldEqual master_seed_hex
    ...
    val pubKeyBytesSmall: IndexedSeq[Byte] = Secp256K1CryptoBC.compressPublicKey(accountKeys.getPublic)

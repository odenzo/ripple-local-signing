...
class RippleFormatConvertersTest extends OTestSpec with FixtureUtils with ByteUtils with RippleFormatConverters {
  ...
  test("PublkeyKey2Address") {
    ...
    def testKey(k: AccountKeys) = {
      ...
      getOrLog(convertMasterKey2masterSeedHex(k.master_key.v.v)) shouldEqual k.master_seed_hex
      getOrLog(convertBase58Check2hex(k.master_seed.v.v)) shouldEqual k.master_seed_hex
      val seedHex = Hex(k.master_seed_hex)
      getOrLog(convertSeedHexToB58Check(seedHex)).v shouldEqual k.master_seed.v.v

      getOrLog(convertBase58Check2hex(k.public_key.v.v)) shouldEqual k.public_key_hex
      getOrLog(convertPubKeyHexToB58Check(Hex(k.public_key_hex))).v shouldEqual k.public_key.v.v

      val pubKey = getOrLog(ByteUtils.hex2bytes(k.public_key_hex))
      getOrLog(accountpubkey2address(pubKey)).v shouldEqual k.account_id.address.v

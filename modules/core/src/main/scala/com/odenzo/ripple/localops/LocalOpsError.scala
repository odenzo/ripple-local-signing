...
/**
  ...
  */
sealed trait LocalOpsError extends Throwable with ErrorUtils {
...
trait ErrorUtils {
  ...
  def wrapDecodingResult[T](v: Result[T], json: Json, note: String = "No Clues"): Either[LOpJsonErr, T] = {
    v.leftMap { err: DecodingFailure =>
      val jsPath = CursorOp.opsToPath(err.history) // Javascript style Path
      LOpJsonErr(note, json, LOpException(s"Decoding Error @ $jsPath ", err).some)
    }
  }
  ...
  def handleM[A](msg: String)(fn: => Either[LocalOpsError, A]): Either[LocalOpsError, A] = {
    Try {
      fn
    } match {
  ...
  def handle[A](msg: String)(fn: => A): Either[LocalOpsError, A] = {
    Try {
      val res: A = fn

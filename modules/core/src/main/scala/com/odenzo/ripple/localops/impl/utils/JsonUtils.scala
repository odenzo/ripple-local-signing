...
trait JsonUtils extends Logging {
  ...
  val droppingNullsPrinter = Printer.spaces2.copy(dropNullValues = true)

  /** Intended to check if an JsonObject. Used in flatTap alot  */
  def ensureJsonObject(a: Json): Either[LOpJsonErr, Boolean] = {
  ...
  def pruneNullFields(obj: Json): Either[LOpException, Json] = {
    // Explicit, but there is no way this should be an error. (Think Circe 12.13 has this via foldable)
    val pretty: String = droppingNullsPrinter.print(obj.asJson)
    parseAsJson(pretty)
